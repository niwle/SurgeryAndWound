                ...
                <div class="card shadow-sm rounded">
                    <div class="card-body">
                        <div class="d-flex mb-3">
                            <h5 class="card-title mr-auto">View Patient : <?php echo strtoupper($row["m_name"]) ?></h5>
                        </div>
                        <div class="card-inner">
                            ...
                            <nav>
                                <div class="nav nav-tabs" id="nav-tab" role="tablist">
                                    ...
                                    <a class="nav-item nav-link" id="nav-contact-tab" data-toggle="tab" href="#photo_comments" role="tab" aria-controls="nav-contact" aria-selected="false">Image Submitted</a>
                                </div>
                            </nav>
                            <div class="tab-content" id="nav-tabContent">
                                <div class="tab-pane fade show active" id="nav-info" role="tabpanel" aria-labelledby="info-tab">
                                    ...
                                    <div align='right'>
                                        ...
                                        <button class="btn btn-danger btn-block view_save" hidden>Save</button>
                                    </div>
                                </div>
                                ...
                                <div class="tab-pane fade" id="photo_comments" role="tabpanel" aria-labelledby="comments-tab">
                                    <table class="table table-striped table-bordered">
                                        ...
                                        <tbody>
                                            <?php
                                            $result = mysqli_query($conn, "SELECT pbe.*, um.m_name as doctor_name FROM progress_book_entry pbe INNER JOIN user_master um on um.m_id = pbe.dcotor_replied WHERE pbe.masterUserid_fk = '" . $pid . "'");
                                            while ($row = mysqli_fetch_assoc($result)) : ?>
                                                    <tr>
                                                        <td><?= $row["created_at"] ?></td>
                                                        <td><img style="width: 50%; height: 50%;" src="../uploads/patient_img/<?= $pid ?>/<?= $row["progressImage"] ?>" alt="No Image">
                                                            <button type='button' data-img='../uploads/patient_img/<?= $pid ?>/<?= $row["progressImage"] ?>' data-toggle='modal' data-target='#exampleModal' style='padding-left:20px; padding-right:20px' class='btn btn-outline-primary'>Click to enlarge</button>
                                                        </td>
                                                        <!-- <td>Dr. <?= $row["doctor_name"] ?></td> -->
                                                        <td><?php $flag = $row["flag"]; echo flag($flag)  ?></td>
                                                        <td>
                                                            <a href="doctor-feedback.php?f=<?php echo base64_encode($row['entryID']) ?>">
                                                            <button type='button'  style='padding-left:20px; padding-right:20px' class='btn btn-outline-warning edit'>View/Feedback</button>
                                                            </a>
                                            ...
                                            <?php if ($result->num_rows == 0) {
                                                    echo '<tr><td colspan="4" align="center">No Result</td></tr>';
                                                }  ?>
                                        </tbody>
                                    </table>
