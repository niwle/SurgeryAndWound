        ...
        if (mysqli_num_rows($result) != 0) {
            ...
            try {
                $sql = 'INSERT INTO user_master 
                        (m_type, m_regis_id, m_name,password, m_identity, m_gender, m_dob, m_age, email, phone, m_address, m_city, m_state, m_zipcode, created_at, updated_at)
                        VALUES ("D","' . $regis_id . '","' . $firstname . '","' . $passwordcombine . '", "' . $ic . '", "' . $gender . '", "' . $dob . '", "' . $age . '", "' . $email . '", "' . $contact . '", "' . $address . '", "' . $city . '", "' . $state . '", "' . $zipcode . '","' . $created_at . '","' . $created_at . '")';
                ...
                if (mysqli_query($conn,$sql)) {
                    ...
                    Swal.fire({ title: "Great!", text: "New Record Added!", type: "success" }).then((result) => {
                        if (result.value) { window.location.href = "patient-doctor-list.php?t='.base64_encode('d').'"; }
                    });
                    </script>';
                }
