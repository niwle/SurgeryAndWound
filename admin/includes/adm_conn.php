...
if (isset($_POST['type'])) {
    ...
    if ($type == "adminUpload") {
        ...
        foreach ($info as $v) {
            ...
            $passwordGenerate_name = strtolower(substr(RemoveSpecialChar($m_name), 0, 4));
            $passwordGenerate_identity = strtolower(substr(RemoveSpecialChar($m_identity), -4));
            $passwordGenerate_regis_id = strtolower(substr(RemoveSpecialChar($m_regis_id), -4));
            ...
            $executeBulkSQL .=
                "INSERT INTO user_master (m_name, password, m_identity, m_type, m_regis_id, m_gender, email, phone, created_at, updated_at) 
            VALUES ('$m_name','$passwordcombine', '$m_identity', '$m_type', '$m_regis_id', '$m_gender', '$email', '$phone', '$current_date', '$current_date');";
        }
        // print_r($executeBulkSQL);
        $result = $conn->multi_query($executeBulkSQL);
    }

    if($type = "removeAppointment"){
        // $appointmentID = $_POST['appointmentID'];
        $appointmentID =  mysqli_real_escape_string($conn, $_POST['appointmentID']);
        $sqlAP = "DELETE FROM appointment WHERE a_id='$appointmentID';";
        $conn->query($sqlAP);
        // echo '1';
    }

    if ($type == "appointmentUpload") {
        function RemoveSpecialChar($str)
        {
        ...
        foreach ($info as $v) {
            $patient_id = mysqli_real_escape_string($conn, $v['patient_id']);
            $doctor_id = mysqli_real_escape_string($conn, $v['doctor_id']);
            $description = mysqli_real_escape_string($conn, $v['description']);
            $apoinment_date = mysqli_real_escape_string($conn, $v['apoinment_date']);


            $executeBulkSQL .=
                "INSERT INTO appointment (patient_id, doctor_id, description, apoinment_date) 
                VALUES ('$patient_id', '$doctor_id', '$description', '$apoinment_date');";
        }
        // print_r($executeBulkSQL);
        $result = $conn->multi_query($executeBulkSQL);
    }

    if($type = "addAppoinment"){
        $input =  mysqli_real_escape_string($conn, $_POST['input']);
        $type =  mysqli_real_escape_string($conn, $_POST['type']);
        $sql = "SELECT * FROM user_master WHERE (m_id LIKE '$input%' OR m_name LIKE '$input%') and m_type = '$type' limit 6;";
        $result = $conn->query($sql);
        $buildlist = "";
        if($result->num_rows == 0){
            $buildlist .= "No result";
        }else{
            while ($rowPatient = $result->fetch_assoc()){
                $p_id = $rowPatient['m_id'];
                $p_name = $rowPatient['m_name'];
                $buildlist.= "<li p_id='$p_id' p_name='$p_name' class='dropdown-item' type='button'>$p_id - $p_name</li>";
            }
        }
        print_r($buildlist);
    ...
    if ($type == "adminEdit") {
        ...
        $m_address = mysqli_real_escape_string($conn, $arr['m_address']);
