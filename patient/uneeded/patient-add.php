...
<body>
    ...
    if (isset($_POST['savebtn'])) {
        ...
        if (mysqli_num_rows($result) != 0) {
            ...
            exit();
        } else if (empty($firstname) && empty($lastname) && empty($ic) && empty($nationality)) {
            echo '<script>
                Swal.fire({ title: "Oops!", text: "Field Cannot be Empty such as Name, IC, Nationality & Avatar !", type: "error" }).then((result) => {
                    if (result.value) { window.location.href = "patient-add.php"; }
                });
            ...
            try {
                $sql = 'INSERT INTO user_master 
                        (m_avatar, m_type,  m_name, m_identity, m_nationality, m_gender, m_dob, m_age, email, phone, m_address, m_city, m_state, m_zipcode, created_at, updated_at)
                        VALUES ("' . $avatars . '","P","' . $firstname . '", "' . $ic . '", "' . $nationality . '", "' . $gender . '", "' . $dob . '", "' . $age . '", "' . $email . '", "' . $contact . '", "' . $address . '", "' . $city . '", "' . $state . '", "' . $zipcode . '","' . $created_at . '","' . $created_at . '")';
