...
if (isset($_POST['loginbtn']))
{
    ...
    $check = $conn->prepare("SELECT * FROM user_master WHERE email = ? and m_type = ?");
    $m_type = 'r';
    $check->bind_param("ss", $inputEmail,$m_type);
    $check->execute();
    $q = $check->get_result();
    $r = $q->fetch_assoc();
    ...
    if(passwordValidator($inputPassword, $dbHashpassword)){
        $_SESSION['sess_rtid'] = $r['m_id'];
        $_SESSION['sess_rtemail'] = $r['email'];
        $_SESSION['rt_loggedin'] = 1;
    ...
    // }
    // else {
    //     $_SESSION['sess_rtid'] = $row['m_id'];
    //     $_SESSION['sess_rtemail'] = $row['email'];
    //     $_SESSION['rt_loggedin'] = 1;
    //     echo "<script>window.location.href = 'index.php'</script>";
    // }
    $stmt->close();
